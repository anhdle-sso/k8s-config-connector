...
function verify_network_quota {
  # when a project is freshly created its reputation is 'low' which results in a network quota limit is 5. Our billing
  # account has a reputation of 'medium' so when the project is associated with our billing account, the cloud abuse
  # reputation service will upgrade the network quota limit to 15. This update is eventually consistent with regards to
  # the billing account assocation. When the system is slow to update our reputation or quota, the tests may fail
  # because they will create more than 5 networks. To know when the quota has been updated, we call this command in a
  # retry loop until the quota is what we expect.
  EXPECTED_NETWORK_QUOTA=15
  # the compute service must be enabled to check the quota
  gcloud services enable compute.googleapis.com
  NETWORK_QUOTA=$(gcloud compute project-info describe --project ${PROJECT_ID} --format json | jq --raw-output '.quotas[] | select(.metric == "NETWORKS") | .limit')
  # launch this in a sub-shell so that 'exit 1' results in an error and doesn't exit the original shell
  (
    if [ "${NETWORK_QUOTA}" -lt "${EXPECTED_NETWORK_QUOTA}" ]; then
      >&2 echo "ERROR: network quota of ${NETWORK_QUOTA} is less than the expected value of ${EXPECTED_NETWORK_QUOTA}"
      exit 1
    fi
  )
}
export -f verify_network_quota
...
echo "Validating network quota..."
retry --max-retries 10 --sleep-seconds 30 --command verify_network_quota
