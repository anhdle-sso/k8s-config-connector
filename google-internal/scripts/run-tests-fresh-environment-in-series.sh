...
SKIP_TESTS_REGEX=""
GO_TEST_SKIP_REGEX=""
PARALLEL="-parallel 1" # Run tests in series by default.
EXIT_ON_ERROR=false
...
while [[ $# -gt 0 ]]; do
  case "${1}" in
    ...
    --parallel)         PARALLEL="-parallel ${2:-}"; shift ;;
...
TEST_FOLDER_ID=${KCC_INTEGRATION_TESTS_FOLDER_ID}  \
  ...
  go test -v ${PARALLEL} ${TARGET_TESTS} ${RUN_TESTS_REGEX} ${SKIP_TESTS_REGEX} ${GO_TEST_RUN_REGEX} ${GO_TEST_SKIP_REGEX} \
