...
function jreport {
  ...
  cat test_int_flaky_log1.txt | ${REPO_ROOT}/hack/convert-to-junit-report > ${ARTIFACTS}/junit_int_report1.xml
}
...
${REPO_ROOT}/google-internal/scripts/run-command-new-env.sh \
  --command "${REPO_ROOT}/google-internal/scripts/run-tests-fresh-environment-in-series.sh \
    ...
    --run-tests '${IAM_FAILED_TESTS_REGEX}|${IAM_FAILED_TEST_FUNCS}' \
